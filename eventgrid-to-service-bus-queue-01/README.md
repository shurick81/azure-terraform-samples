...
```
endpoint=$(az eventgrid topic show --name topic-eventgrid-to-service-bus-queue-lab-01 -g eventgrid-to-service-bus-queue-lab-01 --query "endpoint" --output tsv);
key=$(az eventgrid topic key list --name topic-eventgrid-to-service-bus-queue-lab-01 -g eventgrid-to-service-bus-queue-lab-01 --query "key1" --output tsv);
