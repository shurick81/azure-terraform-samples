...
```
endpoint=$(az eventgrid topic show --name topic-eventgrid-to-service-bus-queue-lab-16 -g eventgrid-to-service-bus-queue-lab-16 --query "endpoint" --output tsv);
key=$(az eventgrid topic key list --name topic-eventgrid-to-service-bus-queue-lab-16 -g eventgrid-to-service-bus-queue-lab-16 --query "key1" --output tsv);
